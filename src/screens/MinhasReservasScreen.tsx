...
import { isReservationHistorico } from '../lib/slots'
...
export function MinhasReservasScreen({ refreshKey, onCancelRequest }: Props) {
  ...
  const ativas = useMemo(() => reservations.filter((r) => !isReservationHistorico(r)), [reservations])
  const historico = useMemo(
    () =>
      reservations
        .filter(isReservationHistorico)
    ...
    <div className="screen">
      <div className="section-header">
        ...
        {!loading && historico.length > 0 && (
          <button type="button" className="btn-pill btn-pill--neutral" onClick={() => setShowHistorico(true)}>
            Histórico
          </button>
        )}
      </div>
      ...
      {!loading && !error && ativas.length === 0 && (
        <div className="empty-state">
          <span className="empty-state__icon">
            <Icon name="local_laundry_service" />
          </span>
          <div className="empty-state__title">Tudo limpo por aqui!</div>
          <div className="empty-state__text">Nenhuma reserva futura neste aparelho.</div>
        </div>
      )}
      ...
      {showHistorico && (
        <HistoricoModal reservations={historico} onClose={() => setShowHistorico(false)} />
      )}
    </div>
  )
}
