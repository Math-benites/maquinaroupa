...
def writeSummary() {
    ...
    sh '''
        ...
        RESULT="${BUILD_RESULT:-EM ANDAMENTO}"
        {
            ...
            echo "  body::before { content:''; position:fixed; top:-30%; left:50%; width:1100px; height:1100px; margin-left:-550px; background:radial-gradient(circle at 30% 30%, rgba(255,94,58,0.35), transparent 60%), radial-gradient(circle at 70% 60%, rgba(123,92,255,0.30), transparent 55%); filter:blur(90px); animation:driftBlob 16s ease-in-out infinite alternate; z-index:0; pointer-events:none; }"
            ...
            echo "  .hero h1 { font-size:clamp(32px,5vw,52px); font-weight:800; line-height:1.08; letter-spacing:-.02em; margin:0 0 18px 0; background:linear-gradient(100deg,#ffffff 20%,#ff8a5c 45%,#b48cff 65%,#ffffff 85%); background-size:300% auto; -webkit-background-clip:text; background-clip:text; color:transparent; animation:shine 6s linear infinite; }"
            echo "  .meta { color:#9a9aa5; font-size:14px; display:flex; flex-wrap:wrap; gap:10px 8px; align-items:center; }"
            echo "  .meta code { background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.08); color:#d7d7de; padding:2px 8px; }"
            echo "  .result-pill { display:inline-block; padding:5px 16px; border-radius:999px; font-size:12px; font-weight:700; letter-spacing:.6px; text-transform:uppercase; box-shadow:0 0 0 1px rgba(255,255,255,0.08) inset; }"
            echo "  .result-pill.success { background:rgba(45,164,78,0.15); color:#5fe08a; box-shadow:0 0 24px rgba(45,164,78,0.35), 0 0 0 1px rgba(95,224,138,0.3) inset; }"
            echo "  .result-pill.failure, .result-pill.unstable { background:rgba(207,34,46,0.15); color:#ff7b7b; box-shadow:0 0 24px rgba(207,34,46,0.35), 0 0 0 1px rgba(255,123,123,0.3) inset; }"
            echo "  .result-pill.em, .result-pill.andamento { background:rgba(154,103,0,0.18); color:#ffc25c; }"
            echo "  .cards { display:flex; flex-direction:column; gap:18px; }"
            echo "  .card { background:linear-gradient(180deg,rgba(255,255,255,0.045),rgba(255,255,255,0.02)); backdrop-filter:blur(6px); border:1px solid rgba(255,255,255,0.08); border-radius:16px; box-shadow:0 20px 40px -20px rgba(0,0,0,0.6); padding:22px 26px; border-left:3px solid #55555f; opacity:0; transform:translateY(22px) scale(.98); filter:blur(6px); animation:cardIn .6s cubic-bezier(.16,1,.3,1) forwards; transition:transform .35s cubic-bezier(.16,1,.3,1), box-shadow .35s ease, border-color .35s ease; }"
            echo "  .card:hover { transform:translateY(-4px) scale(1.005); box-shadow:0 28px 60px -20px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.06) inset; }"
            echo "  .card.ok { border-left-color:#3fd67a; }"
            echo "  .card.ok:hover { box-shadow:0 28px 60px -20px rgba(0,0,0,0.7), 0 0 40px -12px rgba(63,214,122,0.35); }"
            echo "  .card.fail { border-left-color:#ff5c5c; }"
            echo "  .card.fail:hover { box-shadow:0 28px 60px -20px rgba(0,0,0,0.7), 0 0 40px -12px rgba(255,92,92,0.35); }"
            echo "  .card h2 { font-size:17px; font-weight:700; letter-spacing:-.01em; margin:0 0 12px 0; display:flex; align-items:center; gap:10px; color:#f2f2f5; }"
            echo "  .card h3 { font-size:12px; margin:16px 0 8px 0; padding-top:14px; border-top:1px solid rgba(255,255,255,0.08); color:#9a9aa5; text-transform:uppercase; letter-spacing:.6px; font-weight:700; }"
            echo "  .card h3:first-of-type { border-top:none; padding-top:0; margin-top:0; }"
            echo "  .card p { color:#c6c6ce; line-height:1.55; }"
            echo "  .badge { display:inline-block; padding:3px 12px; border-radius:999px; font-size:12px; font-weight:700; }"
            echo "  .badge.ok { background:rgba(63,214,122,0.15); color:#5fe08a; }"
            echo "  .badge.fail { background:rgba(255,92,92,0.15); color:#ff8a8a; }"
            echo "  table { border-collapse:collapse; margin:12px 0; width:100%; max-width:520px; }"
            echo "  th, td { border:1px solid rgba(255,255,255,0.08); padding:8px 14px; text-align:left; font-size:13px; color:#d7d7de; }"
            echo "  th { background:rgba(255,255,255,0.04); color:#9a9aa5; font-weight:600; }"
            echo "  pre { background:#000000; color:#c9d1d9; padding:14px 16px; border-radius:10px; overflow-x:auto; font-size:12px; max-height:420px; border:1px solid rgba(255,255,255,0.08); }"
            echo "  code { background:rgba(255,255,255,0.07); padding:1px 6px; border-radius:5px; font-size:12px; color:#e8e8ec; }"
            echo "  a { color:#ff8a5c; text-decoration:none; }"
            echo "  a:hover { text-decoration:underline; }"
            echo "  @keyframes driftBlob { 0% { transform:translate(-4%,-2%) scale(1); } 100% { transform:translate(4%,4%) scale(1.15); } }"
            echo "  @keyframes shine { to { background-position:300% center; } }"
            echo "  @keyframes heroIn { from { opacity:0; transform:translateY(-16px); } to { opacity:1; transform:translateY(0); } }"
            echo "  @keyframes cardIn { to { opacity:1; transform:translateY(0) scale(1); filter:blur(0); } }"
